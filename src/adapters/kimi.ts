...
export class KimiAdapter extends CloudDocAdapter {
  ...
  private parseDocTable(markdown: string): PriceItem[] {
    ...
    for (const table of tables) {
      ...
      if (columnTitles.length < 2) continue;
      ...
      if (ctxIdx >= 0) skipIndices.add(ctxIdx);
      ...
      while ((rowMatch = rowArrayRegex.exec(rowsMatch[1])) !== null) {
        ...
        for (const priceIdx of priceIndices) {
          if (priceIdx >= cells.length) continue;
   ...
   * Kimi 定价页面列表
   */
  private readonly PRICING_PAGES = [
