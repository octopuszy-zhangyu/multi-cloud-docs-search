...
export class AliyunAdapter extends CloudDocAdapter {
    ...
    async searchDocuments(productId, keyword) {
        ...
        for (const entry of entries) {
            ...
            const match = expandedGroups.every(synonymGroup => {
                ...
                if (originalKw.length >= 2 && entryText.includes(originalKw))
                    return true;
                return false;
            });
            if (match) {
     ...
     * 阿里云价格计算器 API
     */
    static PRICE_API_BASE = "https://buy-api.aliyun.com/pricingDetail";
    /**
     * 从阿里云价格计算器 API 获取 ECS 价格
     * 主要返回实例规格（instance_type）价格，附带系统盘、数据盘和带宽的参考价格
     */
    async fetchEcsPriceFromApi(region = "cn-beijing") {
        const prices = [];
        ...
        const otherResults = await Promise.all(otherItems.map((billItemCode) => this.queryPricingDetailList(billItemCode, defaultRegion, 5).catch((err) => {
     ...
     */
    async queryPricingDetailList(billItemCode, region, maxItems) {
        ...
        do {
            ...
            if (maxItems !== undefined && allPrices.length >= maxItems)
                break;
            // 限制最大页数，防止无限循环
     ...
     * 解析价格查询响应
     */
    parsePricingDetailResponse(data, billItemCode) {
        const prices = [];
        ...
        for (const value of values) {
            // 提取地域信息
            const regionObj = (value["$priceDetailRegionCode$"] || value["region"]);
            const regionName = regionObj?.name || "华北2（北京）";
            // 提取按量付费价格
            const postPay = value.postPay;
     ...
     * 标准化单位格式
     */
    normalizeUnit(rawUnit, billingMode) {
        if (!rawUnit)
            return "元/月";
        // 已经是完整格式
        if (rawUnit.startsWith("元/"))
            return rawUnit;
     ...
     * 构建产品名称
     */
    buildProductName(billItemCode, value) {
        const parts = [];
        switch (billItemCode) {
            case "instance_type": {
                ...
                const cpu = value.cpu;
                const mem = value.mem;
                const family = value.instance_type_family;
            ...
            case "systemdisk": {
                parts.push("系统盘");
                const category = value.systemdisk_category;
                const level = value.systemdisk_performance_level;
                const size = value.systemdisk_size;
                if (category?.name)
                    parts.push(category.name);
                if (level?.name && level.name !== "不区分")
            ...
            case "datadisk": {
                parts.push("数据盘");
                const category = value.datadisk_category;
                const level = value.datadisk_performance_level;
                const size = value.datadisk_size;
                if (category?.name)
                    parts.push(category.name);
                if (level?.name && level.name !== "不区分")
                    parts.push(level.name);
            ...
            case "vm_bandwidth": {
                parts.push("公网带宽");
                const bandwidth = value.vm_bandwidth;
                if (bandwidth?.name)
                    parts.push(bandwidth.name);
                break;
            }
            default:
