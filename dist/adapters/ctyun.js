...
export class CtyunAdapter extends CloudDocAdapter {
    ...
    flavorMapCache = null;
    flavorExpiry = 0;
    deskRegionCache = null;
    deskRegionExpiry = 0;
    deskProductsCache = null;
    deskProductsExpiry = 0;
     ...
     * 从地域列表中查找包含关键字的地域
     */
    findRegionByKeyword(regions, keyword) {
        if (!keyword)
            return null;
     ...
     * 获取天翼云电脑产品规格
     */
    async getDesktopProducts(regionUuid) {
     ...
     * 查询天翼云电脑价格
     */
    async queryDesktopPrice(regionUuid, skuConfig) {
        const cookie = await this.getCtyunCookie();
    ...
    async getEcsPrice(options) {
        ...
        // 6. 解析包月价格
        const parsePrices = (raw, billingMode, unit) => {
            const returnObj = raw?.returnObj;
            if (!returnObj || raw.statusCode !== 800)
                return;
            for (const sub of returnObj.subOrderPrices || []) {
                const flavorInfo = flavorMap.get(sub.flavor_uuid);
                if (!flavorInfo)
                    continue;
                const vmPrice = (sub.orderItemPrices || []).find((p) => p.resourceType === "VM");
                result.prices.push({
    ...
    async getDesktopPrice(options) {
        ...
        try {
            ...
            const cycleLabels = { 3: "包月", 5: "包年" };
            const cycleUnits = { 3: "元/月", 5: "元/年" };
            const tasks = [];
            for (const product of products) {
                for (const series of product.series || []) {
                    for (const sku of series.sku || []) {
                        const tplAttr = (series.attrs || []).find((a) => a.attrKey === "tplId");
                        const tplVals = tplAttr?.attrVals || [];
                        const sysDiskSizeAttr = (series.attrs || []).find((a) => a.attrKey === "sysDiskSize");
                        const sysDiskTypeAttr = (series.attrs || []).find((a) => a.attrKey === "sysDiskType");
                        const sysDiskSize = sysDiskSizeAttr?.attrVal || "80";
                        const sysDiskType = sysDiskTypeAttr?.attrVal || "SAS";
                        const dataDiskTypeAttr = (series.attrs || []).find((a) => a.attrKey === "dataDiskType");
                        const dataDiskSizeAttr = (series.attrs || []).find((a) => a.attrKey === "dataDiskSize");
            ...
            for (let i = 0; i < tasks.length; i += BATCH_SIZE) {
                ...
                const batchResults = await Promise.all(batch.map((task) => this.queryDesktopPrice(targetRegion.id, task.skuConfig).then((prices) => ({
                    task,
                    prices,
                }))));
